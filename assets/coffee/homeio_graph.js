...
(function() {
  ...
  this.HomeIOMeasGraph = (function() {
    function HomeIOMeasGraph() {
      this.getRaw = bind(this.getRaw, this);
      this.buffer = [];
      this.periodicInterval = 2000;
      this.periodicDynamicMultiplier = 20;
      this.periodicDynamicMinimum = 1000;
    ...
    HomeIOMeasGraph.prototype.getInitials = function() {
      if (this.periodicDynamicMultiplier) {
        return $.getJSON("/api/settings.json", (function(_this) {
          return function(backendSetting) {
            var oldInterval;
            oldInterval = _this.periodicInterval;
            _this.periodicInterval = backendSetting.object.meas.cycleInterval * _this.periodicDynamicMultiplier;
            if (_this.periodicInterval < _this.periodicDynamicMinimum) {
              _this.periodicInterval = _this.periodicDynamicMinimum;
            }
            if (_this.periodicInterval !== oldInterval) {
              console.log("interval changed from " + oldInterval + " to " + _this.periodicInterval);
            }
            return _this.getMeas();
          };
        })(this));
      } else {
        return this.getMeas();
    ...
    HomeIOMeasGraph.prototype.getMeas = function() {
      return $.getJSON("/api/meas/" + this.meas_name + "/.json", (function(_this) {
    ...
    HomeIOMeasGraph.prototype.addToBuffer = function(array) {
      ...
      for (j = 0, len = array.length; j < len; j++) {
        ...
        results.push(this.buffer.push(d));
      }
      return results;
    };

    HomeIOMeasGraph.prototype.renderGraph = function() {
      var d, extremeX, i, j, k, len, len1, new_d, new_data, old_data, ref, x, y;
      new_data = [];
      i = 0;
      extremeX = 0;
      ref = this.buffer;
      ...
      if (new_data.length > 0) {
        ...
        if (this.divideX !== 1.0) {
          old_data = new_data;
          new_data = [];
          for (k = 0, len1 = old_data.length; k < len1; k++) {
            d = old_data[k];
